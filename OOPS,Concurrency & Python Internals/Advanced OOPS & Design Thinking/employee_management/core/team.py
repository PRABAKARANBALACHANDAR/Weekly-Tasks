...
class Team (Data ):
    ...
    def to_dict (self ):
        return {'team_id':self ._team_id ,'team_leader_id':self ._team_leader_id ,'members':self ._members ,'department':self ._department ,'level':self ._level }
    ...
    def get_next_team_id ():
        ...
        existing_ids =sorted ([int (k [1 :])for k in teams .keys ()if k .startswith ('T')and len (k )>1 and k [1 :].isdigit ()])
        ...
        return f'T{max (existing_ids )+1 :03d}'

    @classmethod 
    def from_dict (cls ,data ):
        return cls (data ['team_id'],data ['team_leader_id'],data .get ('members',[]),data .get ('department',''),data .get ('level',2 ))
