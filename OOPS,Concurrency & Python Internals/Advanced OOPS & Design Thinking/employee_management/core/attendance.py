...
class Attendance (Data ):
    ...
    @property 
    def id (self ):
        return f'{self ._employee_id }_{self ._date }'

    @property 
    def name (self ):
        return f'Attendance {self ._employee_id } {self ._date }'
    ...
    def to_dict (self ):
        return {'employee_id':self ._employee_id ,'date':self ._date ,'status':self ._status }
    ...
    def update (self ,requester_employee =None ,**kwargs ):
        ...
        if requester_employee :
            employee_records =[(k ,v )for k ,v in attendance_data .items ()if v ['employee_id']==self ._employee_id ]
            ...
            if self ._date !=latest_date :
                raise PermissionError (f'HR can only update the LAST attendance record. Last record date: {latest_date }, Attempted: {self ._date }')
        key =f'{self ._employee_id }_{self ._date }'
    ...
    def get_employee_attendance (employee_id ):
        attendance_data =JSONHandler .load ('attendance.json')
        records =[Attendance .from_dict (v )for v in attendance_data .values ()if v ['employee_id']==employee_id ]
