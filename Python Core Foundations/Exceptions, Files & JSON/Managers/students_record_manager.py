...
class RecordManager(RecordManagerABC):
    ...
    def create_record(self, name, age, grade, course, assigned_teacher=None):
    ...
    def assign_student_to_teacher(self, student_name, teacher_name):
        student_found=False
        for sid, details in self._records.items():
            if details["name"]==student_name.capitalize():
                self._records[sid]["assigned_teacher"]=teacher_name.capitalize()
                student_found=True
                break
        
        if not student_found: return False

        teacher_found=False
        for tid, details in self._teacher_records.items():
            if details["name"]==teacher_name.capitalize():
                if "assigned_students" not in self._teacher_records[tid]:
                     self._teacher_records[tid]["assigned_students"]=[]
                if student_name.capitalize() not in self._teacher_records[tid]["assigned_students"]:
                    self._teacher_records[tid]["assigned_students"].append(student_name.capitalize())
                teacher_found=True
                break
        ...
        if choice=="all":
            ...
            if not self._teacher_records:
                ...
                for id_num, details in sorted(self._teacher_records.items(), key=lambda x: int(x[0])):
                    print(f"ID: {id_num}, Name: {details['name']}, Dept: {details['department']}, Students: {len(details.get('assigned_students', []))}")
        else:
